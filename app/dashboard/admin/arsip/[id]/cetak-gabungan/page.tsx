...
export default async function CetakGabungan(props: Params) {
  ...
  // Get the best file to display (FINAL_SCAN first, fallback to DRAFT)
  const bestFile = doc.files.find(f => f.fileType === "FINAL_SCAN") || doc.files.find(f => f.fileType === "DRAFT");
  const isImage = bestFile ? /\.(jpg|jpeg|png|gif|webp|svg)$/i.test(bestFile.filePath) || bestFile.filePath.includes("data:image") : false;
  const isPdf = bestFile ? /\.pdf$/i.test(bestFile.filePath) : false;
  ...
  return (
    <div className="p-8 max-w-[21cm] mx-auto bg-white min-h-[29.7cm] text-black relative">
      ...
      <div id="print-area">
        {/* --- PAGE 1: LEMBAR DISPOSISI (EXACT MATCH!) --- */}
        <div style={{ border: '4px solid black', backgroundColor: 'white', color: 'black', fontFamily: 'Arial, sans-serif', width: '100%', minHeight: '27.7cm' }}>
          {/* Header */}
        ...
        {/* --- PAGE 2: MAIN DOCUMENT SCAN (WITH PAGE BREAK!) --- */}
        {bestFile && (
          <div className="page-break" style={{ paddingTop: '20px', minHeight: '29.7cm', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'flex-start' }}>
            <h3 className="print:hidden text-gray-500 font-medium mb-4 text-center">--- Lampiran Dokumen Surat ---</h3>
            {isImage ? (
              <img 
                src={bestFile.filePath} 
                alt="Lampiran Surat" 
                style={{ width: '100%', height: 'auto', maxHeight: '28cm', objectFit: 'contain', border: '1px solid #ddd' }} 
              />
            ) : isPdf ? (
              <iframe 
                src={bestFile.filePath} 
                title="Lampiran Surat PDF" 
                style={{ width: '100%', height: '28cm', border: 'none' }}
              />
            ) : (
              <div className="p-8 text-center bg-gray-50 border border-dashed border-gray-300 rounded-xl text-gray-600">
                Berkas lampiran format <strong>{bestFile.filePath.split('.').pop()?.toUpperCase()}</strong> tidak dapat dipratinjau langsung. Silakan download berkas dari detail surat.
              </div>
            )}
          </div>
        )}
